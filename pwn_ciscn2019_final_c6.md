...
2. 注意在ptmalloc使用一个无限循环来申请堆块，因此在malloc_consolidate改变了fastbin和unsortedbin的布局后，又会重新判断更新后的堆布局中是否存在满足用户申请的内存的块，因此会走到这段代码中。由于我们申请的内存为0x78，而unsorted bin 中此时的确存在
```c
/*
